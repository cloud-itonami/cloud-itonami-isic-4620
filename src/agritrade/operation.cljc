(ns agritrade.operation
  "OperationActor -- one agri-wholesale operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (AgriTradeAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Agri Trading Governor
  ...
  One graph run = one agri-wholesale operation (intake -> advise ->
  govern -> decide -> commit | hold | approval). No unbounded inner
  loop -- each operation is auditable and checkpointed. An agri-order's
  ...
  decision to a human operator (trading supervisor / elevator or yard
  operator). The approver resumes with `{:approval {:status :approved}}`
  (or :rejected). `:delivery/dispatch`/`:invoice/settle` ALWAYS reach
  this node when the governor is clean -- see `agritrade.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [agritrade.agritradeadvisor :as agritradeadvisor]
...
(defn build
  ...
  `agritrade.store/Store`).
  opts:
    :advisor      -- an `agritrade.agritradeadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (agritradeadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; AgriTradeAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (agritradeadvisor/-advise advisor store request)]
            {:proposal p :audit [(agritradeadvisor/trace request p)]})))

      ;; Agri Trading Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
